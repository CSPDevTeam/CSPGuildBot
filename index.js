...
function QrCodeLogin() {
  ...
  client.on("system.login.qrcode", (e) => {
    ...
    if (os.type() == "Windows_NT") {
      logger.warn(
        "检测到您在使用Windows系统，如遇无法扫码请输入gbot qrcode打开窗口扫码"
      );
    }
...
function PassWordLogin() {
  ...
  client.on("system.login.slider", (e) => {
    logger.info(
      "本次登录需要滑动验证,请按照提示滑动验证,获取到Ticket后输入gbot ticket <Ticket>即可提交ticket"
    );
  ...
  client.on("system.login.device", (e) => {
    logger.info(
      "本次登录需要设备验证,请按照提示操作,操作完成后请输入gbot login再次登陆"
    );
    logger.info("Url:", e.url);
    variables.needVerify = true;
  });
...
function startQrcodeProgress() {
  if (os.type() == "Windows_NT") {
    let qr_path = `${variables.photo_view} ${process.cwd()}\\data\\${
      variables.account
    }\\qrcode.png`;
...
function queryUpdate() {
  network.httpGet(
    "https://api.github.com/repos/CSPDevTeam/CSPGuildBot/releases/latest",
    (status, result) => {
      if (status == 200) {
        ...
        if (Number(latestVersionNumber) > Number(currentVersionNumber)) {
          ...
          Number(latestVersionNumber) == Number(currentVersionNumber)
        ) {
